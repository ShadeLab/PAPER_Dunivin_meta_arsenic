...
(map.plot <- ggplot() + 
  mapWorld + 
  geom_point(data = meta.qual, aes(x=Lon, y=Lat, color = Site, size = N), alpha = 0.15) +
  geom_point(data = meta.qual, aes(x=Lon, y=Lat, color = Site, size = N), shape = 1) +
  scale_size(range = c(3,10)) + 
  theme_classic() + 
  scale_color_manual(values = c("#808000", "#aa6e28", "#ffe119", "#f58231", "#aaffc3", "#fabebe", "#d2f53c", "#008080", "#3cb44b", "#ffd8b1", "#808080", "#911eb4", "#000080", "#46f0f0", "#0082c8", "grey75")) +
  theme(panel.background = element_rect(fill='grey80')))

ggsave(map.plot, filename = paste(wd, "/figures/site.map.png", sep = ""), units = "in", width = 12, height = 7)
ggsave(map.plot, filename = paste(wd, "/figures/site.map.eps", sep = ""), units = "in", width = 12, height = 7)
